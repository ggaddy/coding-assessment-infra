    ...
    for retry in range(retries):
        try:
            async with httpx.AsyncClient() as client:
                # with auth token
                # response = await client.get(url=api_url, timeout=timeout, headers=header)
...
async def test_get_all_starwars_people() -> None:
    # get all of them from the API but only print out the first 50, to show as an example
